...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var loadFile = function (inZipPath, url) {
    let searchAndHash = "";
    if (url) {
      let [base, search, hash] = utils.splitUrl(url);
      searchAndHash = hash;
    }
    fetchFile({
      inZipPath: inZipPath,
  ...
  var parseDocument = function (doc, inZipPath, onComplete, recurseChain) {
    ...
    var parserDone = function () {
      var content = utils.doctypeToString(doc.doctype) + doc.documentElement.outerHTML;
        ...
        case "img": {
          ...
          if (elem.hasAttribute("srcset")) {
            elem.setAttribute("srcset",
              utils.parseSrcset(elem.getAttribute("srcset"), (url) => {
  ...
  var ComplexUrlFetcher = class ComplexUrlFetcher {
    ...
    getUrlHash(url, rewriteMethod) {
      var key = utils.getUuid();
      this.urlHash[key] = {
        url: url,
        newUrl: null,
        rewriteMethod: rewriteMethod
      };
      return "urn:utils:url:" + key;
    }
    ...
    startFetches(callback) {
      ...
      if (len > 0) {
        keys.forEach((key) => {
          ...
          if (info.inZip) {
            let targetUrl = inZipPathToUrl(info.inZipPath);
            if (this.recurseChain.indexOf(utils.splitUrlByAnchor(targetUrl)[0]) !== -1) {
    ...
    finalRewrite(text) {
      return text.replace(/urn:utils:url:([0-9a-f]{8}-(?:[0-9a-f]{4}-){3}[0-9a-f]{12})/g, (match, key) => {
  ...
  var processCssFile = function (params, callback) {
    ...
    utils.parseCssFile(data, charset, (text, onReplaceComplete) => {
  ...
  var processCssFileText = function (cssText, refUrl, fetcher) {
    var result = utils.parseCssText(cssText, {
  ...
  viewer.addEventListener("load", (e) => {
    ...
    doc.documentElement.addEventListener("click", (e) => {
      ...
      switch (elem.nodeName.toLowerCase()) {
        case "a": case "area":
          try {
            let url = utils.splitUrl(elem.href)[0];
